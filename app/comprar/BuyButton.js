...
const Product = () => {
  ...
  const handleBuy = async () => {
    const id = await createPreference(customerEmail);
    if (id) {
      setPreferenceId(id);
    }
  };
  ...
  return (
    <div>
        <input type="email" placeholder="Ingresa tu correo electrónico" className={styles.emailInput} onChange={e => setCustomerEmail(e.target.email)} /> 
        <button
            onClick={handleBuy}
